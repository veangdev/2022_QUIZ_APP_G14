...
function create_Quiz(parater) {
  // Create the quiz by data structur to html..
  ...
  if (global_Index < parater.length - 1 && number_Click > 0) {
    ...
    document.getElementById("question_Number").textContent =
      global_Index + 1 + "/" + (parater.length - 1);

    if (start.type_of_question == "checkbox") {
      document.getElementById("alert").textContent =
        "There are more than onn correct answer.";
    } else {
      document.getElementById("alert").textContent = "";
    }
    ...
    if (global_Index == 0) {
      ...
      document.getElementById("submith").onclick = function () {
        document.getElementsByClassName("quiz_Container")[0].style.display =
          "none";
        correction();
      };
    }
...
function correction() {
  ...
  getView.textContent =
    "To view your correction, Please click on button below.";
...
for (let click of button_Click) {
  click.addEventListener("click", getClick);
}
...
function menuTask(event) {
  ...
  //
  //
  let contain_button_back_to_homepage_on_page_quiz =
    document.createElement("div");
  contain_button_back_to_homepage_on_page_quiz.className =
    "contain-button-back-to-homepage";
  let button_back_to_homepage_on_page_quiz = document.createElement("button");
  button_back_to_homepage_on_page_quiz.className =
    "back-to-homepage home-page btn-back";
  button_back_to_homepage_on_page_quiz.id = "btnBack";
  button_back_to_homepage_on_page_quiz.textContent = "back";
  button_back_to_homepage_on_page_quiz.addEventListener(
    "click",
    closePageQuiztoHomePage
  );
  contain_button_back_to_homepage_on_page_quiz.appendChild(
    button_back_to_homepage_on_page_quiz
  );
  ...
  button_menu_create_on_page_quiz.addEventListener(
    "click",
    closePageQuiztoCreateQuestion
  );
  ...
  for (i = 0; i < localStorage.length; i++) {
    ...
    var getting_data_from_localStorage = JSON.parse(
    ...
    top.textContent =
      getting_data_from_localStorage[getting_data_from_localStorage.length - 1][
        "title_quiz"
      ];
